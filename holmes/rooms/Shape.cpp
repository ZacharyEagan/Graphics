...
void Shape::loadMesh(const string &meshName)
{
	...
	else if (shapes.size())
	{
      ...
      //{1.0,1.0,1.0};//shapes[0].mesh.normals;
      float normsx, normsy, normsz;
      for (int i = 0; i < eleBuf.size(); i += 3)
      {

         glm::vec3 a = glm::vec3(
            posBuf.at(eleBuf.at(i)*3) - posBuf.at(eleBuf.at(i+1)*3),
            posBuf.at(eleBuf.at(i)*3 + 1) - posBuf.at(eleBuf.at(i+1)*3 + 1),
            posBuf.at(eleBuf.at(i)*3 + 2) - posBuf.at(eleBuf.at(i+1)*3 + 2));
         glm::vec3 b = glm::vec3(
            posBuf.at(eleBuf.at(i)*3) - posBuf.at(eleBuf.at(i+2)*3),
            posBuf.at(eleBuf.at(i)*3 + 1) - posBuf.at(eleBuf.at(i+2)*3 + 1),
            posBuf.at(eleBuf.at(i)*3 + 2) - posBuf.at(eleBuf.at(i+2)*3 + 2));
         glm::vec3 norm = glm::cross(a,b);
         //gen line a
         //gen line b
         //norm = a x b
         //normsx=((float)i/100.01);
         normsx=norm.x;
         normsy=norm.y;
         normsz=norm.z;
         
         //add normal for first vertex
         norBuf.at(eleBuf.at(i)*3) += normsx;
         norBuf.at(eleBuf.at(i)*3 + 1) += normsy;
         norBuf.at(eleBuf.at(i)*3 + 2) += normsz;
      
         //add normal to second vertex
         norBuf.at(eleBuf.at(i + 1)*3) += normsx;
         norBuf.at(eleBuf.at(i + 1)*3 + 1) += normsy;
         norBuf.at(eleBuf.at(i + 1)*3 + 2) += normsz;
         
         //add normal to third vertex
         norBuf.at(eleBuf.at(i + 2)*3) += normsx;
         norBuf.at(eleBuf.at(i + 2)*3 + 1) += normsy;
         norBuf.at(eleBuf.at(i + 2)*3 + 2) += normsz;
      }
