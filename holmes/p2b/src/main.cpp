...
int main(int argc, char **argv)
{
   ...
   Matrix::createTranslateMat(trans, 0, -1, BACKOFF);
   ...
   shapes.push_back(shape);
	...
	prog = application->init(resourceDir, "simple_frag.glsl");
   Matrix::createScaleMat(scale, 1.2, 1.2, 1.2);
   Matrix::createTranslateMat(trans, 0, 1, BACKOFF);
   ...
   shapes.push_back(shape);
	...
	prog = application->init(resourceDir, "simple_frag.glsl");
   Matrix::createScaleMat(scale, 0.8, 0.8, 0.8);
   Matrix::createTranslateMat(trans, 0, 2, BACKOFF);
   Matrix::multMat(MV3, trans, scale);
   MVs.push_back(MV3);
   progs.push_back(prog);
	shape = application->initGeom(resourceDir, "sphere.obj");
   shapes.push_back(shape);
	...
	prog = application->init(resourceDir, "simple_frag.glsl");
   Matrix::createRotateMatX(rot, 1.57079632);
   Matrix::createScaleMat(scale, 8, 1, 8);
   Matrix::createTranslateMat(trans, 0, -4, BACKOFF); 
   Matrix::multMat(temp1, scale, rot);
   Matrix::multMat(MV5, trans, temp1);
   MVs.push_back(MV5);
   progs.push_back(prog);
	//shape = application->initGeom(resourceDir, "sphere.obj");
   ...
   {
      prog = application->init(resourceDir, "simple_frag.glsl");
      Matrix::createScaleMat(scale, 0.2, 0.2, 0.2);
      Matrix::createTranslateMat(trans, rand()%8 - 4,
                                (rand()%100 - 50)/5, 
                                -(rand() % 8 + 6));
     trans[13] = -4 + (((float)(rand() % 2400))/300);  
     trans[12] = 0 + (((float)(rand() % 1800) - 900.00)/300);
     trans[14] = BACKOFF + (((float)(rand() % 1800) - 900.00)/300);

      Matrix::multMat(MV[i], trans, scale);
      MVs.push_back(MV[i]);
      progs.push_back(prog);
      shape = application->initGeom(resourceDir, "bunny.obj");
      shapes.push_back(shape);
   }

	prog = application->init(resourceDir, "trans_frag.glsl");
   Matrix::createScaleMat(scale, 10.0, 10.0, 10.0);
   Matrix::createTranslateMat(trans, 0, 1, BACKOFF);
   Matrix::multMat(MV4, trans, scale);
   MVs.push_back(MV4);
   progs.push_back(prog);
	shape = application->initGeom(resourceDir, "teapot.obj");
   shapes.push_back(shape);
   ...
   gettimeofday(&tim,0);
	...
	while (! glfwWindowShouldClose(windowManager->getHandle()))
	{
      ...
      {
         Matrix::createTranslateMat(gravityTrans, 0, -((random()%1000 + 1) / 100000 + 0.04), 0 );
         if (i > 3 && i < progs.size() - 1)
         {
            Matrix::multMat(tempGT, gravityTrans, MVs.at(i));
            //Matrix::multMat(tempUR, tempID, tempGT);
            //Matrix::multMat(MVs.at(i), application->UnivRot, tempUR);
            Matrix::multMat(MVs.at(i), tempID, tempGT);
           // Matrix::multMat(MVs.at(i), application->UnivRot, tempUR);
            
            if (MVs.at(i)[13] < -3) //Y
            {
               MVs.at(i)[13] = 3;  
               MVs.at(i)[12] = 0 + (((float)(rand() % 1800) - 900.00)/300);
               MVs.at(i)[14] = BACKOFF + (((float)(rand() % 1800) - 900.00)/300);

            }
            //if (MVs.at(i)[12] < -3) //Y
            //{
           //    MVs.at(i)[13] = 3;  
            //   MVs.at(i)[12] += ((rand() %100 -49) /50);
            //   MVs.at(i)[14] += ((rand() %100 -49) /50) - 9;
           // }
           // if (MVs.at(i)[14] > -3) //Y
           // {
           //    MVs.at(i)[13] = 3;  
         ...
         if (i == 2)
         {
           // MVs.at(i)[12] += 0.005 * cos(0.9 * time(NULL));
            timL.tv_usec = tim.tv_usec;
            gettimeofday(&tim, 0);
            //`cout << tim << "\n";
            MVs.at(i)[13] -= 0.15 * cos(timL.tv_usec / 100000);
            MVs.at(i)[13] += 0.15 * cos(tim.tv_usec / 100000);
           // MVs.at(i)[14] += 0.005 * sin(0.9 *time(NULL));
   
         }
             
         Matrix::multMat(tempDEF, tempID, MVs.at(i));
         Matrix::multMat(MVs.at(i), application->UnivRot, tempDEF);
