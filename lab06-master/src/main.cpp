   ...
   float Sholva = 0;
	...
	void render()
	{
		...
		MV->pushMatrix();
			...
			MV->pushMatrix();
				// place at shoulder
				MV->translate(vec3(1.0, 1, 0));
				// rotate shoulder joint
				MV->rotate(sTheta + Sholva, vec3(0, 0, 1));
				// move to shoulder joint
				MV->translate(vec3(1.0, 0, 0));
				// non-uniform scale

            
            MV->pushMatrix();
               MV->translate(vec3(0.75,0.0,0));
               MV->rotate(sTheta + 0.1 + Elb, vec3(0,0,1));
               MV->translate(vec3(0.75, 0, 0));

                    MV->pushMatrix();
                       MV->translate(vec3(0.7,0.0,0));
                       MV->rotate(sTheta + 0.1, vec3(0,0,1));
                       MV->translate(vec3(0.27, 0, 0));
                       MV->scale(vec3(.26, 0.25, 0.25));
                       glUniformMatrix4fv(prog->getUniform("MV"), 1, 
                         GL_FALSE, value_ptr(MV->topMatrix()));
                       shape->draw(prog);
                    MV->popMatrix();


               MV->scale(vec3(.75, 0.25, 0.25));
				   glUniformMatrix4fv(prog->getUniform("MV"), 1, 
                 GL_FALSE, value_ptr(MV->topMatrix()));
               shape->draw(prog);
			   MV->popMatrix();
				
         MV->scale(vec3(1.0, 0.25, 0.25));
...
/////////
			MV->pushMatrix();
				...
				MV->translate(vec3(-1.0, 1, 0));
				// rotate shoulder joint
				MV->rotate(sTheta + 3.14159, vec3(0, 0, -1));
				// move to shoulder joint
				MV->translate(vec3(1.0, 0, 0));
				// non-uniform scale
		...
		MV->pushMatrix();
			...
			MV->translate(vec3(Shuffle, 1.1, -5));
